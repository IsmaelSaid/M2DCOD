...
print("Somme des éléments de chaques colonnes:")
...
getRatings(anime_ratings_real_ratings_matrix)

#nombre de rating
nratings(anime_ratings_real_ratings_matrix) 
#nombre de rating par utilisateur
#rowCounts(anime_ratings_real_ratings_matrix)
#nombre de rating par item
#colCounts(anime_ratings_real_ratings_matrix)
...
# Normalisation
La normalisation tente de réduire le biais de notation individuel en centrant les données par ligne.
On a le choix d'utiliser une normalisation par centrage (défaut) ou par zscore.
Par centre: On soustrait chaque note disponible de la moyenne des notes de cet utilisateur (ligne)
On peut normaliser les notes avec la fonction normalize.
...
getRatings(nrrmx_anime_ratings) 
#normalisation par z-score
nzrrmx_anime_ratings <- normalize(anime_ratings_real_ratings_matrix, method="Z-score")
getRatings(nzrrmx_anime_ratings) 
```
...
```{r}
recommender <- Recommender(anime_ratings_real_ratings_matrix, method = "popular")
```
