...
as(nrrmx_anime_ratings[1:3,1:10], "list")
```
...
## Histogramme
```{r,echo=TRUE}
hist(getRatings(rrm_anime),breaks=100)
```
...
```{r, fig.width = 5,echo=TRUE, fig.height=3.7}
...
hist(getRatings(nrrmx_anime_ratings), breaks=10,
     xlab ="notes",
     main="répartitions des notes normalisés",
     labels = TRUE)
```
...
colCounts(rrm_anime [1,]) 
#afficher les notes du premier user
